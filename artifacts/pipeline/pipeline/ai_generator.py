...
def _language_instruction(options: dict) -> str:
    ...
    return (
        ...
        "content at all.\n"
        "IMPORTANT: some of the raw Sunsky source data you're given "
        "(specs, compatibility notes, option lists) may itself be in a "
        "language OTHER than English -- Russian, Chinese, or another "
        "language entirely, not just English. Confirmed via a real "
        "generated product: a whole sentence of Russian source text "
        "(\"Этот аксессуар предназначен исключительно для...\") was "
        "copied verbatim into an otherwise-Bulgarian description instead "
        "of being translated. This is wrong every time it happens, "
        "regardless of which language the leftover text is in: read "
        "and understand source data in ANY language, then write your "
        "OWN Bulgarian sentence conveying that same meaning -- never "
        "copy a source sentence or phrase verbatim into the output "
        "just because you don't recognize its language or aren't sure "
        "how to translate it. The ONLY exceptions to \"everything is in "
        "Bulgarian\" are the specific brand-name/model-code tokens "
        "described above -- nothing else, in any language, should ever "
        "appear untranslated in the final text.\n"
        "IMPORTANT: this is a translation of MEANING, not of WORDS. "
        "Client feedback confirmed live on a real generated product "
        "(DCA9918B, a charging case/box for spare GoPro batteries): "
        "generated Bulgarian content read like an awkward, literal, "
        "word-for-word rendering of the raw English name (something "
        "like \"charger box\", unnatural in Bulgarian) rather than "
        "natural copy a Bulgarian native speaker would actually write. "
        "Before writing anything, first understand what the product "
        "actually IS and does -- its real function, category, and use "
        "case -- from the name, specs, and category context together, "
        "the same way an experienced Bulgarian e-commerce copywriter "
        "would after being briefed on the product, not the way a "
        "translator would after being handed only a sentence to "
        "convert word-by-word with no context. Then write ORIGINAL "
        "Bulgarian sentences describing that understanding in whatever "
        "phrasing is most natural and idiomatic in Bulgarian -- even "
        "if that means restructuring the sentence, choosing a "
        "completely different word than the English one's literal "
        "dictionary equivalent, or describing the same feature in a "
        "different order than the source text does."
    )
