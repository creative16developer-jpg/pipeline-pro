        ...
        <div className="flex items-center justify-between pt-3 border-t border-border">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Go back to Process to re-apply current Image Settings"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium bg-card border border-border text-foreground/70 hover:bg-background"
            >
              <RotateCcw className="w-3 h-3" />
              Back to Process
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          onClick={async () => {
            if (!confirm("Go back to Process? This re-runs image processing for this pipeline's products, then continues forward through the pipeline again.")) return;
            try {
              const r = await fetch(`/api/pipelines/${pl.id}/back-to-process`, { method: "POST" });
              if (!r.ok) throw new Error(await r.text());
              toast({ title: "Back to Process" });
...
function ContentReviewSection({ pl, onDone }: { pl: Pipeline; onDone: () => void }) {
  ...
  // Process has no review screen of its own -- this re-runs it
  // directly, then the pipeline flows forward through Enrich/Generate/
  // Cat.Review again the normal way once it's done.
